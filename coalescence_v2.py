...
# set elements of matrix c below a threshold to 0
c_0 = [item for sublist in params['c'].values.tolist() for item in sublist]
c_0 = 10**np.quantile(np.log10(c_0),0.1) # this is the threshold
for i in range(np.sum(assumptions['SA']) + assumptions['Sgen']):
    for a in range(np.sum(assumptions['MA'])):
        if params['c'].iloc[i,a] < c_0:
            params['c'].iloc[i,a] = 0
        
'''
# set random elements of c to 0
assumptions_binary = assumptions.copy()
assumptions_binary['sampling'] = 'Binary'
assumptions_binary['muc'] = 5
c_binary = community_simulator.usertools.MakeMatrices(assumptions_binary)[0]

params['c'] = np.multiply(params['c'],c_binary)
'''
...
'''
delete these lines later vvv
'''
np.sum(N_invasive.iloc[:,0])
np.sum(N_invasive.iloc[:,1])
np.sum(N_invasive.iloc[:,2])
np.sum(N_invasive.iloc[:,3])
np.sum(N_invasive.iloc[:,4])
'''
delete these lines later ^^^
'''

### RE-STABILIZE COHORTS (dominant removed)

# make cohort plates (same as invasive/domintant plates, dominant abundance set to 0)
invasive_cohorts_plate = invasive_plate.copy()
invasive_cohorts_plate.N = invasive_cohorts_plate.N.droplevel(0)

resident_cohorts_plate = resident_plate.copy()
resident_cohorts_plate.N = resident_cohorts_plate.N.droplevel(0)

for i in range(assumptions['n_wells']):
    invasive_cohorts_plate.N.loc[dominants_invasive[i]][i] = 0
    resident_cohorts_plate.N.loc[dominants_resident[i]][i] = 0
invasive_cohorts_plate.N.index = invasive_plate.N.index
resident_cohorts_plate.N.index = resident_plate.N.index

N0_invasive_cohorts = invasive_cohorts_plate.N.copy()
N0_resident_cohorts = resident_cohorts_plate.N.copy()

# stabilize
N_invasive_cohorts, R_invasive_cohorts = stabilizeCommunities(invasive_cohorts_plate)
N_resident_cohorts, R_resident_cohorts = stabilizeCommunities(resident_cohorts_plate)
...
# get top-down cohesiveness (invasive communities)
# top-down cohesiveness (t) is defined as the relative change in cohort biomass
# when the dominant is present versus when it is absent
t_invasive = ['NA' for i in range(assumptions['n_wells'])]
t_resident = ['NA' for i in range(assumptions['n_wells'])]
for i in range(assumptions['n_wells']):
    
    # community compositions as lists (R: resident, I: invasive)
    R = N_resident.iloc[:,i].tolist()
    I = N_invasive.iloc[:,i].tolist()
    
    # total biomass of invasive cohort
    I_cohort = np.sum(N0_invasive_cohorts.iloc[:,i])
    
    # total biomass of invasive cohort when dominant is removed and community re-stabilized
    I_nodom_cohort = np.sum(N_invasive_cohorts.iloc[:,i])
