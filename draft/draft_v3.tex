...
Several parameters can modulate the strength and direction of community cohesiveness,
...
non-overlapping:
\iffalse
(i.e. $\mathrm{s}_1$ and $\mathrm{s}_2$ secretions
differ from $\mathrm{s}_3$ and $\mathrm{s}_4$ secretions)
\fi
...
we first stabilized each community separately
...
invasions from the invasive dominant alone.
...
\paragraph*{Top-down resident versus top-down invasive.}
...
can only be 0 (if the resident dominant ``wins'' competition for
$\mathrm{R}_1$) or 1 (if the opposite happens).
...
outcompetes the resident dominant ($\mathrm{s}_1$) for the primary resource ($c_{11}>c_{31}$).

\paragraph*{Bidirectional resident versus top-down invasive.}
Blablabla...

\paragraph*{Top-down resident versus bidirectional invasive.}
Blablabla...

\paragraph*{Bidirectional resident versus bidirectional invasive.}
Blablabla...
...
\textbf{b.}
\iffalse
Simulations with a Microbial Consumer-Resource Model suggest that positive bottom-up
